...
class OrderController extends Controller
{
    ...
    public function manageOrders(Request $request, Order $order)
    {
        ...
        $validStatuses = ['Pending', 'Out for delivery', 'Delivered'];
        ...
        $log_entry = Auth::user()->fname . " marked as " . $statusUpdated . " for " .  $order->user->fname . "'s order. " . " with the id# " . $order->id;
    ...
    public function createOrderNow(Request $request)
    {
        ...
        if ($product->stock == 0) {
            ...
            $log_entry = Auth::user()->fname . " has ordered a product: " . $productName . " for " . $order->user->fname . " with the id# " . $order->id;
            event(new UserLog($log_entry));
